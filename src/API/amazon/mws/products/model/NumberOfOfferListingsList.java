
package API.amazon.mws.products.model;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for NumberOfOfferListingsList complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="NumberOfOfferListingsList">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="OfferListingCount" type="{http://mws.amazonservices.com/schema/Products/2011-10-01}OfferListingCountType" maxOccurs="unbounded"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "NumberOfOfferListingsList", propOrder = {
    "offerListingCount"
})
public class NumberOfOfferListingsList {

    @XmlElement(name = "OfferListingCount", required = true)
    protected List<OfferListingCountType> offerListingCount;

    /**
     * Default constructor
     * 
     */
    public NumberOfOfferListingsList() {
        super();
    }

    /**
     * Value constructor
     * 
     */
    public NumberOfOfferListingsList(final List<OfferListingCountType> offerListingCount) {
        this.offerListingCount = offerListingCount;
    }

    /**
     * Gets the value of the offerListingCount property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the offerListingCount property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getOfferListingCount().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link OfferListingCountType }
     * 
     * 
     */
    public List<OfferListingCountType> getOfferListingCount() {
        if (offerListingCount == null) {
            offerListingCount = new ArrayList<OfferListingCountType>();
        }
        return this.offerListingCount;
    }

    public boolean isSetOfferListingCount() {
        return ((this.offerListingCount!= null)&&(!this.offerListingCount.isEmpty()));
    }

    public void unsetOfferListingCount() {
        this.offerListingCount = null;
    }

    /**
     * Sets the value of the OfferListingCount property.
     * 
     * @param values
     * @return
     *     this instance
     */
    public NumberOfOfferListingsList withOfferListingCount(OfferListingCountType... values) {
        for (OfferListingCountType value: values) {
            getOfferListingCount().add(value);
        }
        return this;
    }

    /**
     * Sets the value of the offerListingCount property.
     * 
     * @param offerListingCount
     *     allowed object is
     *     {@link OfferListingCountType }
     *     
     */
    public void setOfferListingCount(List<OfferListingCountType> offerListingCount) {
        this.offerListingCount = offerListingCount;
    }
    


    /**
     * 
     * XML fragment representation of this object
     * 
     * @return XML fragment for this object. Name for outer
     * tag expected to be set by calling method. This fragment
     * returns inner properties representation only
     */
    public String toXMLFragment() {
        StringBuffer xml = new StringBuffer();
        java.util.List<OfferListingCountType> offerListingCountList = getOfferListingCount();
        for (OfferListingCountType offerListingCount : offerListingCountList) {
            xml.append("<OfferListingCount " + (offerListingCount.isSetCondition() ? " condition=" + "\"" + escapeXML(offerListingCount.getCondition()) + "\"" : "") + ">");
            xml.append(offerListingCount.getValue());
            xml.append("</OfferListingCount>");
        }
        return xml.toString();
    }

    /**
     * 
     * Escape XML special characters
     */
    private String escapeXML(String string) {
        if (string == null)
            return "null";
        StringBuffer sb = new StringBuffer();
        int length = string.length();
        for (int i = 0; i < length; ++i) {
            char c = string.charAt(i);
            switch (c) {
            case '&':
                sb.append("&amp;");
                break;
            case '<':
                sb.append("&lt;");
                break;
            case '>':
                sb.append("&gt;");
                break;
            case '\'':
                sb.append("&#039;");
                break;
            case '"':
                sb.append("&quot;");
                break;
            default:
                sb.append(c);
            }
        }
        return sb.toString();
    }



    /**
     *
     * JSON fragment representation of this object
     *
     * @return JSON fragment for this object. Name for outer
     * object expected to be set by calling method. This fragment
     * returns inner properties representation only
     *
     */
    protected String toJSONFragment() {
        StringBuffer json = new StringBuffer();
        boolean first = true;
        if (isSetOfferListingCount()) {
            if (!first) json.append(", ");
            json.append("\"OfferListingCount\" : [");
            java.util.List<OfferListingCountType> offerListingCountList = getOfferListingCount();
            int offerListingCountListIndex = 0;
            for (OfferListingCountType offerListingCount : offerListingCountList) {
                if (offerListingCountListIndex > 0) json.append(", ");
                json.append("{");
                json.append("");
                json.append(offerListingCount.getValue());
                json.append("}");
                first = false;
                ++offerListingCountListIndex;
            }
            json.append("]");
        }
        return json.toString();
    }

    /**
     *
     * Quote JSON string
     */
    private String quoteJSON(String string) {
        if (string == null)
            return "null";
        StringBuffer sb = new StringBuffer();
        sb.append("\"");
        int length = string.length();
        for (int i = 0; i < length; ++i) {
            char c = string.charAt(i);
            switch (c) {
            case '"':
                sb.append("\\\"");
                break;
            case '\\':
                sb.append("\\\\");
                break;
            case '/':
                sb.append("\\/");
                break;
            case '\b':
                sb.append("\\b");
                break;
            case '\f':
                sb.append("\\f");
                break;
            case '\n':
                sb.append("\\n");
                break;
            case '\r':
                sb.append("\\r");
                break;
            case '\t':
                sb.append("\\t");
                break;
            default:
                if (c <  ' ') {
                    sb.append("\\u" + String.format("%03x", Integer.valueOf(c)));
                } else {
                sb.append(c);
            }
        }
        }
        sb.append("\"");
        return sb.toString();
    }


}
